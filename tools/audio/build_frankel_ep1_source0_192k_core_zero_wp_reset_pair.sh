#!/usr/bin/env bash
# Build, but never flash, the Frankel EP1 192 kHz / AoC zero-wp-reset pair.
...
trial_root="$project_root/work/audio-research/frankel/speaker-ep1-source0-192k-core-zero-wp-reset-pair"
output_dir=${1:-"$trial_root/trial-1"}

base_vkb="$base_dir/vendor_kernel_boot.img"
base_vbmeta="$base_dir/vbmeta.img"
patcher="$script_dir/patch_frankel_aoc_core_zero_wp_reset.py"
...
readonly clean_alsa_sha=d0278904c384a61d9d25a44236a3e4a36c087be077b2bd6f0b971ab74b2b3cf8
readonly stock_core_sha=23acc08d0539657e72a0bc506abf6cef9950b90b192c51fd0dd9bf2177e4f2ad
readonly patched_core_sha=f4b7c9daad2fb3cb2ddc9fa8f80381629b3ffe048194348924e9f7a0ead1024c
...
candidate="$stage/aoc_core.zero-wp-reset.ko"
require_sha "$alsa_module" "$clean_alsa_sha"
require_sha "$core_module" "$stock_core_sha"
python3 "$patcher" "$core_module" "$candidate"
require_sha "$candidate" "$patched_core_sha"
install -m 0644 -- "$candidate" "$core_module"
python3 "$patcher" "$core_module" --check patched
...
lz4 -q -f -l -12 -- "$stage/vendor_ramdisk.patched.cpio" \
  "$stage/final/vendor_ramdisk00.core-zero-wp-reset"
...
"$mkbootimg" \
  ...
  --vendor_ramdisk_fragment "$stage/final/vendor_ramdisk00.core-zero-wp-reset" \
...
{
  printf 'variant=frankel-ep1-source0-192k-core-zero-wp-reset\n'
  ...
  printf 'alsa_module_sha256=%s\n' "$clean_alsa_sha"
  printf 'aoc_core_patch=0xa190:ff060071->1f040071 cmp_w23_size_minus_wp_to_cmp_w0_wp\n'
