#!/usr/bin/env bash
# Build, but never flash, the Frankel EP1/source-0 192 kHz VKB/vbmeta pair.
...
trial_root="$project_root/work/audio-research/frankel/speaker-ep1-source0-192k-pair"
output_dir=${1:-"$trial_root/trial-1"}

base_vkb="$base_dir/vendor_kernel_boot.img"
base_vbmeta="$root_base_dir/vbmeta-powerphone.img"
patcher="$script_dir/patch_frankel_aoc_ep1_playback_192k.py"
...
readonly patched_module_sha=d0278904c384a61d9d25a44236a3e4a36c087be077b2bd6f0b971ab74b2b3cf8
...
python3 "$patcher" "$module" "$stage/aoc_alsa_dev_util.ep1-source0-192k.ko"
require_sha "$stage/aoc_alsa_dev_util.ep1-source0-192k.ko" "$patched_module_sha"
install -m 0644 -- "$stage/aoc_alsa_dev_util.ep1-source0-192k.ko" "$module"
touch -d @0 -- "$module"
...
lz4 -q -f -l -12 -- "$stage/vendor_ramdisk.patched.cpio" \
  "$stage/final/vendor_ramdisk00.ep1-source0-192k"
...
"$mkbootimg" \
  ...
  --ramdisk_name '' \
  --vendor_ramdisk_fragment "$stage/final/vendor_ramdisk00.ep1-source0-192k" \
...
cp -f -- "$stage/aoc_alsa_dev_util.ep1-source0-192k.ko" \
  "$stage/final/aoc_alsa_dev_util.ep1-source0-192k.ko"
{
  printf 'variant=frankel-ep1-source0-192k\n'
  printf 'pcm_device=0\n'
  printf 'aoc_service=audio_playback0\n'
  printf 'aoc_source=0\n'
  printf 'module_patch=0x3f368:fe000000->fe1f0000\n'
  printf 'new_vendor_kernel_boot_digest=%s\n' "$new_digest"
  (
    cd -- "$stage/final"
    sha256sum -- aoc_alsa_dev_util.ep1-source0-192k.ko \
