#!/usr/bin/env bash
# Build, but never flash, Frankel's EP1/source-0 D0 timer-only pair.
...
trial_root="$project_root/work/audio-research/frankel/speaker-ep1-source0-192k-d0-timer-only-1ms-pair"
output_dir=${1:-"$trial_root/trial-1"}

base_vkb="$base_dir/vendor_kernel_boot.img"
base_vbmeta="$base_dir/vbmeta.img"
patcher="$script_dir/patch_frankel_aoc_ep1_d0_timer_only_1ms.py"
...
readonly patched_module_sha=e24c6c20dddc055108bcd1ce841106d21e7251eff0f5eb8bd349d7a315f43cbb
...
candidate="$stage/aoc_alsa_dev_util.ep1-source0-192k-prefill-d0-timer-only-1ms.ko"
python3 "$patcher" "$module" "$candidate"
require_sha "$candidate" "$patched_module_sha"
install -m 0644 -- "$candidate" "$module"
python3 "$patcher" --check d0-timer-only-1ms "$module"
...
lz4 -q -f -l -12 -- "$stage/vendor_ramdisk.patched.cpio" \
  "$stage/final/vendor_ramdisk00.ep1-source0-192k-d0-timer-only-1ms"
...
"$mkbootimg" \
  ...
  --ramdisk_name '' \
  --vendor_ramdisk_fragment "$stage/final/vendor_ramdisk00.ep1-source0-192k-d0-timer-only-1ms" \
...
cp -f -- "$candidate" \
  "$stage/final/aoc_alsa_dev_util.ep1-source0-192k-prefill-d0-timer-only-1ms.ko"
{
  printf 'variant=frankel-ep1-source0-192k-d0-ring-prefill-d0-timer-only-1ms\n'
  printf 'base_variant=frankel-ep1-source0-192k-d0-ring-prefill\n'
  printf 'pcm_device=0\n'
  printf 'aoc_service=audio_playback0\n'
  printf 'aoc_source=0\n'
  printf 'progress_mode=D0 timer only; mailbox IRQ private data cleared\n'
  printf 'host_timer_interval_ns=1000000\n'
  printf 'other_main_pcm_selection=stock; fallback timer interval is 1 ms\n'
  ...
  (
    cd -- "$stage/final"
    sha256sum -- aoc_alsa_dev_util.ep1-source0-192k-prefill-d0-timer-only-1ms.ko \
