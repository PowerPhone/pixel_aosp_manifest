#!/usr/bin/env bash
# Build, but never flash, Frankel's EP1/source-0 D0 bounded-consumption pair.
...
trial_root="$project_root/work/audio-research/frankel/speaker-ep1-source0-192k-d0-consumed-step-pair"
output_dir=${1:-"$trial_root/trial-2"}

base_vkb="$base_dir/vendor_kernel_boot.img"
base_vbmeta="$base_dir/vbmeta.img"
patcher="$script_dir/patch_frankel_aoc_ep1_d0_consumed_step.py"
...
readonly patched_module_sha=4e1be77f4e60596e0e16a64dc448cb201b0f297b2b79efab6aec50049a89b129
...
candidate="$stage/aoc_alsa_dev_util.ep1-source0-192k-prefill-d0-consumed-step.ko"
python3 "$patcher" "$module" "$candidate"
require_sha "$candidate" "$patched_module_sha"
install -m 0644 -- "$candidate" "$module"
python3 "$patcher" --check d0-consumed-step "$module"
...
lz4 -q -f -l -12 -- "$stage/vendor_ramdisk.patched.cpio" \
  "$stage/final/vendor_ramdisk00.ep1-source0-192k-d0-consumed-step"
...
"$mkbootimg" \
  ...
  --ramdisk_name '' \
  --vendor_ramdisk_fragment "$stage/final/vendor_ramdisk00.ep1-source0-192k-d0-consumed-step" \
...
cp -f -- "$candidate" \
  "$stage/final/aoc_alsa_dev_util.ep1-source0-192k-prefill-d0-consumed-step.ko"
{
  printf 'variant=frankel-ep1-source0-192k-d0-ring-prefill-d0-consumed-step\n'
  printf 'base_variant=frankel-ep1-source0-192k-d0-pure-timer-1ms\n'
  printf 'pcm_device=0\n'
  printf 'aoc_service=audio_playback0\n'
  printf 'aoc_source=0\n'
  printf 'progress_mode=D0 pure timer plus bounded consumed-counter stepping\n'
  printf 'host_timer_interval_ns=1000000\n'
  printf 'other_main_pcm_selection=stock; D0 reported consumption never exceeds hardware counter\n'
  ...
  (
    cd -- "$stage/final"
    sha256sum -- aoc_alsa_dev_util.ep1-source0-192k-prefill-d0-consumed-step.ko \
