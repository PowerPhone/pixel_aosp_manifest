#!/usr/bin/env bash
# Build, but never flash, the D0 real-mailbox/direct-period kernel experiment.
...
trial_root="$project_root/work/audio-research/frankel/speaker-ep1-source0-192k-d0-mailbox-direct-period-elapsed-pair"
output_dir=${1:-"$trial_root/trial-1"}

base_vkb="$base_dir/vendor_kernel_boot.img"
patcher="$script_dir/patch_frankel_aoc_ep1_d0_mailbox_direct_period_elapsed.py"
...
readonly patched_module_sha=fe401845555d26edc29128d544265bf27b537ba7bdaabe8fd50dfa4b658be928
...
candidate="$stage/aoc_alsa_dev_util.ep1-source0-192k-d0-mailbox-direct-period-elapsed.ko"
...
python3 "$patcher" "$module" --check d0-mailbox-direct-period-elapsed
...
lz4 -q -f -l -12 -- "$stage/vendor_ramdisk.patched.cpio" \
  "$stage/final/vendor_ramdisk00.ep1-source0-192k-d0-mailbox-direct-period-elapsed"
...
{
  printf 'variant=frankel-ep1-source0-192k-d0-mailbox-direct-period-elapsed\n'
  printf 'base_variant=frankel-ep1-source0-192k-d0-mailbox-real-progress\n'
  printf 'pcm_device=0\n'
  printf 'aoc_service=audio_playback0\n'
  printf 'aoc_source=0\n'
  printf 'progress_mode=D0 stock mailbox ISR; max one real AoC Rx period per notification\n'
  ...
  printf 'period_delivery=D0 synchronous snd_pcm_period_elapsed; non-D0 stock workqueue\n'
