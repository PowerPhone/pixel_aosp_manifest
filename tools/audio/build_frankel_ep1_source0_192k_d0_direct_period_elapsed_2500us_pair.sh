#!/usr/bin/env bash
# Build, but never flash, Frankel's D0 direct-period 2.5 ms image pair.
...
base_dir="$project_root/work/audio-research/frankel/speaker-ep1-source0-192k-d0-pure-timer-1ms-pair/trial-1"
trial_root="$project_root/work/audio-research/frankel/speaker-ep1-source0-192k-d0-direct-period-elapsed-2500us-pair"
output_dir=${1:-"$trial_root/trial-1"}

base_vkb="$base_dir/vendor_kernel_boot.img"
base_vbmeta="$base_dir/vbmeta.img"
clock_patcher="$script_dir/patch_frankel_aoc_ep1_d0_direct_period_clock_2500us.py"
delivery_patcher="$script_dir/patch_frankel_aoc_ep1_d0_direct_period_elapsed_2500us.py"
...
readonly base_vkb_sha=1c869c6df2fd2d67f7a43fee129b718aecb77f802b394a5a0b1a4569b4ae4fe8
readonly base_vbmeta_sha=0a0a8c970e96fa525683615e7c06b6f0486e38906ddbe26e94e594d2932f57d1
readonly base_module_sha=053900e08ffb5fb62ce7b4929f3bf6ef1db3d66991d4824ee37a008563f5f42f
readonly clock_module_sha=1ce8ca36e915d5c3cd05d5e5f731f5374caf9f4c349111dadb01c4ed684f84e2
readonly patched_module_sha=cc82b4ebdd4b5f758c7ebc7fd1d58d4264e1fc54b68b52e3761a6590c160513c
readonly old_root_vkb_digest=531de582b3d37873cb6001203a88dbb089d0a3621b686e36d58e741c3b954eab
...
for path in "$clock_patcher" "$delivery_patcher" "$unpack_bootimg" \
  "$mkbootimg" "$avbtool" "$avb_key"; do
  [[ -f "$path" && ! -L "$path" ]] || die "missing or unsafe input: $path"
done
...
clock_candidate="$stage/aoc_alsa_dev_util.d0-direct-period-clock-2500us.ko"
candidate="$stage/aoc_alsa_dev_util.ep1-source0-192k-prefill-d0-direct-period-elapsed-2500us.ko"
require_sha "$module" "$base_module_sha"
python3 "$clock_patcher" "$module" "$clock_candidate"
require_sha "$clock_candidate" "$clock_module_sha"
python3 "$delivery_patcher" "$clock_candidate" "$candidate"
require_sha "$candidate" "$patched_module_sha"
install -m 0644 -- "$candidate" "$module"
python3 "$delivery_patcher" "$module" --check d0-direct-period-elapsed
...
lz4 -q -f -l -12 -- "$stage/vendor_ramdisk.patched.cpio" \
  "$stage/final/vendor_ramdisk00.ep1-source0-192k-d0-direct-period-elapsed-2500us"
...
{
  printf 'variant=frankel-ep1-source0-192k-d0-direct-period-elapsed-2500us\n'
  printf 'base_variant=frankel-ep1-source0-192k-d0-pure-timer-1ms\n'
  printf 'pcm_device=0\n'
  printf 'aoc_service=audio_playback0\n'
  printf 'aoc_source=0\n'
  printf 'progress_mode=D0 2.5ms logical clock; direct snd_pcm_period_elapsed delivery\n'
  printf 'host_timer_interval_ns=2500000\n'
  printf 'period_delivery=D0 direct; non-D0 stock workqueue\n'
  ...
  (
    cd -- "$stage/final"
    sha256sum -- "$(basename -- "$candidate")" vendor_kernel_boot.img vbmeta.img
  )
