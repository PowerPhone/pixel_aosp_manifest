...
while IFS= read -r existing_manifest; do
  [[ "$existing_manifest" == pixel-devices.xml || \
...
if [[ -e .repo/local_manifests/cubs.xml || -L .repo/local_manifests/cubs.xml ]]; then
  [[ -f .repo/local_manifests/cubs.xml && \
     ! -L .repo/local_manifests/cubs.xml ]] || \
    die "legacy cubs local manifest is not a safe regular file"
  rm -f -- .repo/local_manifests/cubs.xml
  note "retired legacy cubs-only local manifest name"
fi
[[ ! -L .repo/local_manifests/pixel-devices.xml ]] || \
  die "Pixel device local manifest must not be a symbolic link"
install -m 0644 "$project_root/manifests/pixel-devices.xml" \
  .repo/local_manifests/pixel-devices.xml
...
if [[ -f "$resolved_manifest" ]] && cmp -s "$resolved_manifest" "$temporary_manifest"; then
...
elif [[ "${PIXEL_AOSP_UPDATE_SOURCE_LOCK:-${CUBS_UPDATE_SOURCE_LOCK:-}}" == 1 ]]; then
  ...
  printf 'review the revision delta, then refresh intentionally with '\''PIXEL_AOSP_UPDATE_SOURCE_LOCK=1 %s'\''\n' \
    "$0" >&2
  exit 1
fi
